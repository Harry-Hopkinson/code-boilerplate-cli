...
(async () => {
    ...
    if (language == "c") {
        ...
        let javaScriptConfig = await readFile(configFiles[language]).catch(console.log);
        const javaScriptFolderPath = path.join(process.cwd(), "javascript.js");
        await writeFile(javaScriptFolderPath, javaScriptConfig.toString()).catch((err) => {
            console.log(err);
            process.exit();
        });
    }
    else if (language == "py") {
        let pythonConfig = await readFile(configFiles[language]).catch(console.log);
        const pythonFolderPath = path.join(process.cwd(), "python.py");
        await writeFile(pythonFolderPath, pythonConfig.toString()).catch((err) => {
            console.log(err);
            process.exit();
        });
    }
    else if (language == "ts") {
        let typescriptConfig = await readFile(configFiles[language]).catch(console.log);
        const typescriptFolderPath = path.join(process.cwd(), "typescript.ts");
        await writeFile(typescriptFolderPath, typescriptConfig.toString()).catch((err) => {
            console.log(err);
            process.exit();
        });
    }
